...
do
    local _cfgOk, _cfgRaw = pcall(function() return readfile("SayeeHub_Config.json") end)
    local _cfgData = nil
...
local _FH_SAVE_PATH = "SayeeHub_Config.json"
...
do
    ...
    ToggleHandlers.base_esp = function(state)
        if state then
            ...
            task.spawn(function()
                while state and _G.SayeeHubAlive do
                    task.wait(3)
                    local stale = false
...
pcall(function()
    if game.CoreGui:FindFirstChild("SayeeHub") then
        game.CoreGui.SayeeHub:Destroy()
    end
end)
T = {
    ...
    TabInact    = Color3.fromRGB(120, 120, 120),
...
do
    ...
    _G._FH_AccentA = _restoreRGB(Config.theme.a, 255, 255, 255)
    _G._FH_AccentB = _restoreRGB(Config.theme.b, 200, 200, 200)
end
...
BannerDev.Text              = "discord.gg/sayeehub"
...
BannerDev.TextColor3        = Color3.fromRGB(255, 255, 255)
BannerDev.ZIndex            = 51
...
Dot.Parent           = Hdr
Corner(Dot, 4)
TitleLbl = Label(Hdr, "Sayee Hub", 14, T.White, Enum.Font.GothamBold)
...
CombatTab   = CreateTab(isMobile and "MAIN" or "SAYEE [MAIN]")
...
CreateToggle(CombatTab.scroll, "GRIM Actions Panel",       "Quick actions: kick self, ragdoll, rejoin.",    function(v) FA.setGRIMActionsVisible(v) end)
...
CreateSection(MiscTab.scroll, "Sayee Customization")
do
            ...
            while _G._LoggerProtecterActive and task.wait(0.05) do
                ...
                or not tradePrompt.Enabled then
                    ...
                    if detections >= 3 then
                        ...
                        lp:Kick("Protected By SayeeHub :) | Username: " .. otherUser)
                        break
                    end
                else
                    detections = 0
...
CreateToggle(MiscTab.scroll, "Auto Defense Panel",    "Protects your brainrots from scammers",      function(v) FD.setGRIMDefenseVisible(v) end)
do
...
FA.FABorderFrame.Name             = "SayeeActionsGradBorder"
...
FA.FAWin.Name             = "SayeeActionsPanel"
...
FA.setGRIMActionsVisible = function(vis)
...
FD.FDBorderFrame.Name             = "GRIMDefenseGradBorder"
...
FD.FDWin.Name             = "GRIMDefensePanel"
...
FD.FDTitleLbl = Label(FD.FDHdr, "GRIM Defense", 11, T.White, Enum.Font.GothamBold)
...
FD.setGRIMDefenseVisible = function(vis)
...
task.spawn(function()
    while _G.SayeeHubAlive and task.wait(0.2) do
        local pg = Players.LocalPlayer:FindFirstChild("PlayerGui")
...
local function _FH_InitMainPill()
    ...
    pillBorder.Name                   = "GRIMPillBorder"
...
task.defer(function()
    ...
    if m.pill_x or m.pill_y then
        local _pillBorder = GUI:FindFirstChild("GRIMPillBorder")
        if _pillBorder then
...
task.spawn(function()
    if not _FH_SavedConfig then
        pcall(function() ShowToggleNotification("GRIM Hub - No saved config", false) end)
    ...
    if m.fa_open  and FA  and FA.setGRIMActionsVisible    then pcall(function() FA.setGRIMActionsVisible(true)     end) end
    if m.fd_open  and FD  and FD.setGRIMDefenseVisible    then pcall(function() FD.setGRIMDefenseVisible(true)     end) end
...
Player.CharacterAdded:Connect(function()
    ...
    if m2.pill_x or m2.pill_y then
        local _pb = GUI:FindFirstChild("GRIMPillBorder")
        if _pb then
    ...
    do
        ...
        function ST:SendWebhook(animalData, plotIdHint)
            ...
            local _embedTitle = _isOgPing
                and "OG HAS BEEN STOLEN WITH GRIM!"
                or  "**👑 Brainrot Stolen!**"
            ...
            local _fields = {
                ...
                { name = "Script",        value = "```SAYEE HUB```",                         inline = true },
            }
            if _userMention then
            ...
            local payload = {
                ...
                embeds = {{
                    ...
                    title = _embedTitle,
                    fields = _fields,
                    footer = { text = "SAYEE HUB • STEAL TRACKER" },
