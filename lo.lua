...
local _updateMobileAutoLeft, _updateMobileAutoRight
...
local function setupSpeedIndicator(char)
	...
	speedLabel.Text="Speed: 0";speedLabel.TextColor3=Color3.fromRGB(200,200,200)
	...
	discordLabel.Text="discord.gg/kekeployd";discordLabel.TextColor3=Color3.fromRGB(200,200,200)
...
local function saveConfig()
	...
	if writefile then pcall(function() writefile("vmrhub.json",HS:JSONEncode(cfg)) end) end
end
...
local function createMobilePanel()
	...
	local function createButton(text, isToggle, callback, initialActive)
		...
		local function setActive(state)
			active = state
			if isToggle then
				container.BackgroundColor3 = active and Color3.fromRGB(70,70,90) or Color3.fromRGB(18,18,24)
			end
		end
		setActive(active)
		btn.MouseButton1Down:Connect(function()
			if not isToggle then
				container.BackgroundColor3 = Color3.fromRGB(70,70,90)
		...
		btn.MouseButton1Click:Connect(function()
			if isToggle then
				...
				setActive(newState)
				if callback then callback(newState, setActive) end
			else
	...
	local tpBtn = createButton("TP\nDOWN", false, function(setActive) runTPFloor() end, false)
	...
	autoLeftBtn.BG.Parent = row1
	local row2 = Instance.new("Frame")
	row2.Size = UDim2.new(1, 0, 0, 62)
	...
	local dropBtn = createButton("DROP", false, function(setActive) runDrop() end, false)
	dropBtn.BG.Parent = row2
	local row3 = Instance.new("Frame")
	row3.Size = UDim2.new(1, 0, 0, 62)
	...
	local carrySpeedBtn = createButton("CARRY", true, function(state, setActive)
		toggleCarryMode()
		setActive(speedMode)
		saveConfig()
	end, speedMode)
	carrySpeedBtn.BG.Parent = row3
	local autoBatBtn = createButton("AIMBOT", true, function(state, setActive)
	...
	autoBatBtn.BG.Parent = row3
	local row4 = Instance.new("Frame")
	row4.Size = UDim2.new(1, 0, 0, 62)
	...
	row4Layout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	row4Layout.Padding = UDim.new(0, 6)
	local instaResetBtn = createButton("RESET", false, function(setActive) cursedInstaReset() end, false)
	instaResetBtn.BG.Parent = row4
	local laggerNormalBtn = createButton("LAGGER", true, function(state, setActive)
		if state then
			speedMode = false
			laggerToggled = true
			laggerPhase = 1
		...
		refreshSpeedModeLabel()
		saveConfig()
	end, laggerToggled and laggerPhase == 1)
	laggerNormalBtn.BG.Parent = row4
...
-- ============================================================
--  НОВЫЙ UI — ЧЁРНО-СЕРЫЙ, ПРЯМОУГОЛЬНЫЙ
-- ============================================================
local function buildGui()
	local BG    = Color3.fromRGB(12,12,16)
	local BG2   = Color3.fromRGB(18,18,24)
	local CARD  = Color3.fromRGB(24,24,32)
	local HOV   = Color3.fromRGB(36,36,48)
	local RED   = Color3.fromRGB(200,200,200)
	local REDDIM= Color3.fromRGB(160,160,160)
	local STROKE= Color3.fromRGB(40,40,50)
	local W     = Color3.fromRGB(220,220,220)
	local DIM   = Color3.fromRGB(140,140,150)
	local INP   = Color3.fromRGB(30,30,40)
	local OFF   = Color3.fromRGB(20,20,28)

	local old=game:GetService("CoreGui"):FindFirstChild("CandyHub")
	if old then old:Destroy() end
	local pg=LP:FindFirstChild("PlayerGui")
	if pg then local o=pg:FindFirstChild("CandyHub")
	if o then o:Destroy() end end
	...
	closeBtn.MouseButton1Click:Connect(function()
		local miniBtn=main.Parent and main.Parent:FindFirstChild("MiniBtn")
		if miniBtn then main.Visible=false;miniBtn.Visible=true end
	end)

	local miniBtn=Instance.new("TextButton",gui)
	miniBtn.Size=UDim2.new(0,90,0,30)
	miniBtn.Position=UDim2.new(0,20,0,20)
	miniBtn.BackgroundColor3=BG2
	miniBtn.BorderSizePixel=0
	miniBtn.Text="VMR HUB"
	miniBtn.TextColor3=RED
	miniBtn.Font=Enum.Font.GothamBold
	miniBtn.TextSize=13
	miniBtn.ZIndex=20
	miniBtn.Visible=false
	Instance.new("UICorner",miniBtn).CornerRadius=UDim.new(0,0)
	drag(miniBtn)
	...
	sf.Size=UDim2.new(1,-16,1,-56)
	sf.Position=UDim2.new(0,8,0,52)
	sf.BackgroundTransparency=1
	sf.BorderSizePixel=0
	sf.ScrollBarThickness=2
	sf.ScrollBarImageColor3=STROKE
	...
	local function mkRow(h)
		...
		f.BorderSizePixel=0
		f.LayoutOrder=LO()
		f.MouseEnter:Connect(function() f.BackgroundColor3=HOV end)
		f.MouseLeave:Connect(function() f.BackgroundColor3=CARD end)
		return f
	end
	...
	local function mkPill(row,offset)
		...
		pill.Position=UDim2.new(1,-offset,0.5,-9)
		pill.BackgroundColor3=OFF
		pill.BorderSizePixel=0
		Instance.new("UICorner",pill).CornerRadius=UDim.new(0,0)
		local dot=Instance.new("Frame",pill)
		dot.Size=UDim2.new(0,14,0,14)
		dot.Position=UDim2.new(0,3,0.5,-7)
		dot.BackgroundColor3=DIM
		dot.BorderSizePixel=0
		Instance.new("UICorner",dot).CornerRadius=UDim.new(0,0)
		return pill,dot
	end

	local function animPill(pill,dot,on)
		pill.BackgroundColor3 = on and Color3.fromRGB(60,60,80) or OFF
		dot.Position = on and UDim2.new(1,-17,0.5,-7) or UDim2.new(0,3,0.5,-7)
		dot.BackgroundColor3 = on and RED or DIM
	end
	...
	local function mkBox(parent,default,w,xOff,cb)
		...
		tb.Size=UDim2.new(0,w,0,20)
		tb.Position=UDim2.new(1,-xOff,0.5,-10)
	...
	mkSect("Speed")
	do local row=mkRow(30);mkLabel(row,"Normal");normalBox=mkBox(row,NS,50,56,function(v) if v>0 then NS=v end;saveConfig() end) end
	do local row=mkRow(30);mkLabel(row,"Carry");carryBox=mkBox(row,CS,50,56,function(v) if v>0 then CS=v end;saveConfig() end) end
	do local row=mkRow(30);mkLabel(row,"Lagger");laggerBox=mkBox(row,LAGGER_SPEED,50,56,function(v) if v>0 then LAGGER_SPEED=v end;saveConfig() end) end
	do local row=mkRow(30);mkLabel(row,"L Carry");laggerCarryBox=mkBox(row,LAGGER_CARRY_SPEED,50,56,function(v) if v>0 then LAGGER_CARRY_SPEED=v end;saveConfig() end) end
	do
		local row=mkRow(30)
		mkLabel(row,"Mode")
		modeValLbl=Instance.new("TextLabel",row)
		modeValLbl.Size=UDim2.new(0,80,1,0)
		modeValLbl.Position=UDim2.new(1,-88,0,0)
		modeValLbl.BackgroundTransparency=1
		modeValLbl.Text="Normal"
		modeValLbl.TextColor3=RED
		modeValLbl.Font=Enum.Font.GothamBold
		modeValLbl.TextSize=10
	...
	setBatCounterVisual=mkToggle("Bat Counter",function(on) batCounterEnabled=on;if on then startBatCounter() else stopBatCounter() end;saveConfig() end)
	...
	mkSect("Steal")
	setInstaGrab=mkToggle("Auto Steal",function(on) Steal.AutoStealEnabled=on;if on then pcall(startAutoSteal) else stopAutoSteal() end;saveConfig() end)
	do local row=mkRow(30);mkLabel(row,"Radius");radInput=mkBox(row,Steal.StealRadius,50,56,function(v) if v>=0.5 then Steal.StealRadius=v end;saveConfig() end) end

	mkSect("Movement")
	setInfJumpVisual=mkToggle("Inf Jump",function(on) infJumpEnabled=on;saveConfig() end)
	...
	do local row=mkRow(30);mkLabel(row,"TP Down");local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,58,0,20);btn.Position=UDim2.new(1,-66,0.5,-10);btn.BackgroundColor3=INP;btn.BorderSizePixel=0;btn.Text=KB.TPFloor.kb and KB.TPFloor.kb.Name or "F";btn.TextColor3=W;btn.Font=Enum.Font.GothamBold;btn.TextSize=10;btn.MouseButton1Click:Connect(function() runTPFloor() end) end
	setAutoTPVisual=mkToggle("Auto TP",function(on) autoTPEnabled=on;if on then startAutoTP() else stopAutoTP() end;saveConfig() end)
	do local row=mkRow(30);mkLabel(row,"TP Height");autoTPHeightBox=mkBox(row,autoTPHeight,50,56,function(v) if v>=0 then autoTPHeight=v end;saveConfig() end) end
	...
	mkSect("Misc")
	do local row=mkRow(30);mkLabel(row,"Instant Reset");local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,68,0,22);btn.Position=UDim2.new(1,-76,0.5,-11);btn.BackgroundColor3=INP;btn.BorderSizePixel=0;btn.Text="RESET";btn.TextColor3=W;btn.Font=Enum.Font.GothamBold;btn.TextSize=10;btn.MouseButton1Click:Connect(function() cursedInstaReset() end) end
	...
	Instance.new("UICorner",pbFrame).CornerRadius=UDim.new(0,0)
	drag(pbFrame)

	progressPct=Instance.new("TextLabel",pbFrame)
	progressPct.Size=UDim2.new(0,44,0,16)
	progressPct.Position=UDim2.new(0,8,0,3)
	progressPct.BackgroundTransparency=1
	progressPct.Text="IDLE"
	progressPct.TextColor3=DIM
	progressPct.Font=Enum.Font.GothamBold
	progressPct.TextSize=9

	progressRadLbl=Instance.new("TextLabel",pbFrame)
	progressRadLbl.Size=UDim2.new(0,60,0,16)
	progressRadLbl.Position=UDim2.new(1,-68,0,3)
	progressRadLbl.BackgroundTransparency=1
	progressRadLbl.Text="R:60"
	progressRadLbl.TextColor3=DIM
	...
	pbg.Size=UDim2.new(1,-16,0,4)
	pbg.Position=UDim2.new(0,8,1,-8)
	pbg.BackgroundColor3=INP
...
local function loadConfigKeys()
	if not(isfile and isfile("vmrhub.json")) then return end
	local ok,cfg=pcall(function() return HS:JSONDecode(readfile("vmrhub.json")) end)
...
local function loadConfigState()
	...
	if progressRadLbl then progressRadLbl.Text="R:"..tostring(Steal.StealRadius) end
